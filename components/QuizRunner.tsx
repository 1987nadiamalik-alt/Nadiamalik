...
const QuizRunner: React.FC<QuizRunnerProps> = ({ questions, settings, onComplete, onExit }) => {
  ...
  useEffect(() => {
    if (settings.enableAudio && status === 'running' && !showAnswer && !isTimeUp) {
      let textToSpeak = "";
      if (currentQuestion.category === 'addition') {
        textToSpeak = currentQuestion.rows.map(r => (r > 0 ? `plus ${r}` : `minus ${Math.abs(r)}`)).join(", ");
      } else {
        textToSpeak = `${currentQuestion.rows[0]} times ${currentQuestion.rows[1]}`;
      }
      playTTS(textToSpeak);
    }
  }, [currentIndex, status, showAnswer, isTimeUp]);
  ...
  useEffect(() => {
    // High-precision timer (100ms interval) to support decimals and smoother bars
        ...
        <div className="flex-1 bg-slate-200 h-2 rounded-full overflow-hidden">
          <div 
            className={`h-full transition-all duration-100 ease-linear ${
              timeLeft <= 1 ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-orange-600 shadow-[0_0_8px_rgba(234,88,12,0.3)]'
            }`}
              ...
              {currentQuestion.category === 'addition' ? (
                <div className="flex flex-col items-center space-y-0.5 w-full">
                  {currentQuestion.rows.map((row, idx) => (
                    <div key={idx} className={`abacus-font ${getFontSize()} font-black text-slate-800 tabular-nums leading-[1.05]`}>
                      {row > 0 ? `+${row}` : row}
                    </div>
                  ))}
                </div>
